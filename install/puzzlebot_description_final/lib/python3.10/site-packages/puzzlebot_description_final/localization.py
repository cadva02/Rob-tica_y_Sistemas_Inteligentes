...
class EkfLocalization(Node):
    ...
    def _update(self, observation: np.ndarray, distance: float | None = None) -> None:
        ...
        # threshold: chi-square approx for 3 DOF (p~0.95 -> 7.8); use slightly laxer threshold
        mahal_threshold = 9.21
        if mahalanobis > mahal_threshold:
            try:
                self.get_logger().warning(
                    f'Rejecting ArUco update: mahalanobis={mahalanobis:.2f} > {mahal_threshold}, dist={dist:.2f}m'
                )
            except Exception:
                pass
            return
